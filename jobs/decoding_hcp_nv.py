...
def _get_parser():
    parser = add_common_job_args(argparse.ArgumentParser(description="Decode HCP NeuroVault maps"))
    parser.add_argument(
        "--image_dir",
        dest="image_dir",
        default=None,
        help="Optional explicit path to the HCP NeuroVault image directory.",
    )
    parser.add_argument(
        "--output_dir",
        dest="output_dir",
...
def main(
    ...
    output_dir = (
        op.join(results_dir, "predictions_hcp_nv") if output_dir is None else op.abspath(output_dir)
    )
